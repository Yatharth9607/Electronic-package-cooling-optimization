%Resistance Network Function
   ...
   function [total_cost, dT_chip] = res_network(Nb,p,db)
...
%{
    if (p - db) < 0.0002
        disp('Diameter of Solder balls is too large compared to Pitch')
        return
    end
%}
    hb = 0.0004;
...
%    [A.B] = sym2poly([eqn1, eqn2, eqn3, eqn4, eqn5, eqn6, eqn7], [q_2, q_3, q_4, q_5, q_6, T_chip, T_sb]);
    q = linsolve(A,B);
    dq = double(q);
    %disp(dq)
